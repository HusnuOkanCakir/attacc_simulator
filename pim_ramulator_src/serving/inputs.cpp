...
void validate_static_config(const FrontendConfig& config) {
  ...
  if (config.hybrid_command_source != "template_replay" &&
      config.hybrid_command_source != "runtime_generated") {
    throw ConfigurationError("RealtimeServingFrontend: hybrid_command_source must be template_replay or runtime_generated");
  }
  if (config.hybrid_command_source == "runtime_generated") {
    if (config.generator_backend != "lpddr5_bank") {
      throw ConfigurationError("RealtimeServingFrontend: runtime_generated only supports generator_backend=lpddr5_bank");
    }
    if (config.translation_max_addr == 0 || config.kv_pool_bytes == 0) {
      throw ConfigurationError("RealtimeServingFrontend: runtime_generated requires translation_max_addr and kv_pool_bytes");
    }
    if (config.translation_page_size_bytes != 4096) {
      throw ConfigurationError("RealtimeServingFrontend: runtime_generated lpddr5_bank requires translation_pagesize_KB=4");
    }
  }
