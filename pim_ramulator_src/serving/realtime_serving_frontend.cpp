...
class RealtimeServingFrontend : public IFrontEnd, public Implementation {
 ...
 public:
  void init() override {
    ...
    config.hybrid_command_source = param<std::string>("hybrid_command_source")
                                       .desc("Hybrid command source: template_replay or runtime_generated.")
                                       .default_val("template_replay");
    ...
    config.generator_backend = param<std::string>("generator_backend").default_val("lpddr5_bank");
    config.generator_dhead = param<int>("generator_dhead").default_val(128);
    config.generator_heads_per_hbm = param<int>("generator_heads_per_hbm").default_val(1);
    config.generator_dtype_bytes = param<int>("generator_dtype_bytes").default_val(2);
    config.generator_num_layers = param<int>("generator_num_layers").default_val(1);
    config.generator_channel_count = param<int>("generator_channel_count").default_val(16);
    config.translation_max_addr = param<Addr_t>("translation_max_addr").default_val(0);
    config.translation_page_size_bytes =
        (param<Addr_t>("translation_pagesize_KB").default_val(4) << 10);
    config.kv_pool_bytes = param<uint64_t>("kv_pool_bytes").default_val(0);
    config.score_pool_bytes = param<uint64_t>("score_pool_bytes").default_val(0);
    config.context_pool_bytes = param<uint64_t>("context_pool_bytes").default_val(0);
    config.scratch_pool_bytes = param<uint64_t>("scratch_pool_bytes").default_val(0);
    ...
    config.kv_placement_csv = param<std::string>("kv_placement_csv").default_val("");
    config.allocator_pressure_csv = param<std::string>("allocator_pressure_csv").default_val("");
    ...
    if (config.hybrid_command_source == "runtime_generated") {
      ...
      auto* control = dynamic_cast<IAttAccServingTranslationControl*>(m_translation);
      if (control == nullptr) {
        throw ConfigurationError(
            "RealtimeServingFrontend: Translation child must implement AttAcc serving control");
      }
      control->attach_state(m_runtime->shared_address_space());
