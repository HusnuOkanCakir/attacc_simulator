  ...
  def score_mac(addr_offset, L):
    ...
    for n_idx in range(math.ceil(L / n_pch / n_rank / n_bg)):# 16 
      ...
      for k_idx in range(math.ceil(dhead / n_mac)): # 2
        ...
        col_idx = idx % (int(LPDDR_GS['row'] / LPDDR_GS['col']))
        num_cols = int(idx / (int(LPDDR_GS['row'] / LPDDR_GS['col'])))
        bank_idx = num_cols % n_bank
        row_idx  = int(num_cols / n_bank)

        # Same bank command (rank)
        for lch in range(math.ceil(valid_channel)):
          addr = addr_offset + lch * LPDDR_GS['ch'] + bank_idx * LPDDR_GS['ba'] + \
                 row_idx * LPDDR_GS['row'] + col_idx * LPDDR_GS['col']
          hex_addr = hex(addr)[2:]
      ...
      if n_idx % 16 == 15 or n_idx == math.ceil(L / n_pch / n_rank / n_bg) - 1:
        ...
        for bg_idx in range(n_bg):   
          for rank in range(n_rank):
            for lch in range(math.ceil(valid_channel)):
              addr = addr_offset + lch * LPDDR_GS['ch'] + rank * LPDDR_GS['rank'] + \
                          bg_idx * LPDDR_GS['bg']
              hex_addr = hex(addr)[2:]
...
def main():
  ...
  print("------   Make a trace of bankgroup-level AttAcc (LPDDR5)   ------")
