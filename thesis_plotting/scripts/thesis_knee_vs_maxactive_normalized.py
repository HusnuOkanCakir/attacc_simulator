#!/usr/bin/env python3
"""Normalized-latency knee curve for the max_active sweep, offered-RPS x-axis.

Mirror of thesis_knee_vs_bs_normalized.py for the maxactive sweep.
"""
...
OUT_NAME = "fig_knee_vs_maxactive_normalized"
...
def render(rows, out_dir: Path, out_name: str = OUT_NAME, linear: bool = False):
    ...
    for ax, idx, title in [
        ...
        for i, kv in enumerate(knob_values):
            ...
            ax.plot(xs, ys, marker=markers[i % len(markers)], ms=9, lw=2.4,
                    color=colors[kv], label=f"max_active = {kv}",
    ...
    fig.suptitle("Pi0 @ azure_poisson_wide  —  normalized knee vs max_active",
