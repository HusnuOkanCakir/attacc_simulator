#!/usr/bin/env python3
"""Normalized-latency knee curve for the route sweep (hybrid vs gpu_only).

Mirror of thesis_knee_vs_route_offered.py but with ms/generated-token
on y instead of raw e2e p99. Two-panel: mean and p99. Offered-RPS
on x. The finite-trace plateau visible in the raw-p99 variant
factors out under normalization, giving the geometric-cliff shape.

Source: cluster_outputs/online_serving_runs/knee_vs_route_20260606_001144/
Output: thesis_plotting/figures/fig_knee_vs_route_normalized.{pdf,png}
...
def load_cells(sweep_dir: Path):
    ...
    for cell in sorted(sweep_dir.iterdir()):
        ...
        rows.append((route, scale, offered, mean_norm, p99_norm))
    return rows
...
def render(rows, out_dir: Path, out_name: str, linear: bool):
    configure_plotting()
    by_route = {}
    for route, scale, offered, mean_n, p99_n in rows:
        by_route.setdefault(route, []).append((offered, mean_n, p99_n, scale))
    ...
    for ax, idx, title in [
        ...
        ax.text(SAFE_HYBRID, 0.02, "  safe op  1.91",
                transform=ax.get_xaxis_transform(),
                ha="left", va="bottom",
                color="#1f77b4", fontsize=FONT_ANNOTATE, fontweight="bold")
        ax.axvline(CEILING_HYBRID, color="#c0392b", linestyle=":",
                   linewidth=1.3, alpha=0.7, zorder=3)
        ax.text(CEILING_HYBRID, 0.02, "  hybrid  2.10",
                transform=ax.get_xaxis_transform(),
                ha="left", va="bottom",
                color="#c0392b", fontsize=FONT_ANNOTATE, fontweight="bold")
        ax.axvline(CEILING_GPUONLY, color="#888", linestyle=":",
                   linewidth=1.3, alpha=0.7, zorder=3)
        ax.text(CEILING_GPUONLY, 0.02, "  gpu-only  0.93",
                transform=ax.get_xaxis_transform(),
                ha="left", va="bottom",
                color="#555", fontsize=FONT_ANNOTATE, fontweight="bold")
    ...
    fig.suptitle("Pi0 @ azure_poisson_wide  —  normalized knee vs route",
...
def main():
    ...
    ap.add_argument("--out-name", default="fig_knee_vs_route_normalized")
    args = ap.parse_args()
