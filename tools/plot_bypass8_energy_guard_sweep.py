...
FILENAME_RE = re.compile(r"replay_summary_guard(?P<tag>[0-9p]+)\.json$")
...
def load_rows(summary_dir: Path) -> pd.DataFrame:
    rows = []
    for path in sorted(summary_dir.glob("replay_summary_guard*.json")):
        m = FILENAME_RE.match(path.name)
        if not m:
            continue
        guard_ms = tag_to_value(m.group("tag"))
        with path.open() as f:
            data = json.load(f)
    ...
    if not rows:
        raise FileNotFoundError(f"No replay_summary_guard*.json files found in {summary_dir}")
    return pd.DataFrame(rows).sort_values("guard_ms").reset_index(drop=True)



def plot_lines(df: pd.DataFrame, cols: list[str], labels: list[str], title: str, ylabel: str, out_path: Path) -> None:
    fig, ax = plt.subplots(figsize=(8, 5))
    for col, label in zip(cols, labels):
        ax.plot(df["guard_ms"], df[col], marker="o", linewidth=2, label=label)
    ax.set_xlabel("Energy latency guard (ms)")
...
def main() -> int:
    ap = argparse.ArgumentParser(description="Plot energy-latency-guard sweep results on bypass8 debug runs.")
    ap.add_argument("--summary-dir", type=Path,
                    default=Path("cluster_outputs/bypass8_energy_guard_sweep"))
    ap.add_argument("--out-dir", type=Path,
                    default=Path("cluster_outputs/bypass8_energy_guard_sweep_plots"))
    ap.add_argument("--prefix", default="bypass8_energy_guard_sweep")
    args = ap.parse_args()
    ...
    plot_lines(df,
               ...
               "Route Fractions vs Energy Guard",
    ...
    plot_lines(df,
               ...
               "Decode Energy Per Token vs Energy Guard",
    ...
    plot_lines(df,
               ...
               "Admission Outcomes vs Energy Guard",
               "Count",
    ...
    plot_lines(df,
               ...
               "Queue Pressure vs Energy Guard",
               "Value",
