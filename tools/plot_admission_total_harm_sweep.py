...
FILENAME_RE = re.compile(r"replay_summary_harm(?P<tag>[0-9p]+)\.json$")
...
def load_rows(summary_dir: Path) -> pd.DataFrame:
    rows = []
    for path in sorted(summary_dir.glob("replay_summary_harm*.json")):
        m = FILENAME_RE.match(path.name)
        if not m:
            continue
        total_harm_ms = tag_to_value(m.group("tag"))
        with path.open() as f:
            data = json.load(f)
    ...
    if not rows:
        raise FileNotFoundError(f"No replay_summary_harm*.json files found in {summary_dir}")
    return pd.DataFrame(rows).sort_values("total_harm_ms").reset_index(drop=True)
...
def main() -> int:
    ap = argparse.ArgumentParser(description="Plot admission_max_total_harm_ms sweep results.")
    ap.add_argument("--summary-dir", type=Path,
                    default=Path("cluster_outputs/azure_admission_total_harm_sweep"))
    ap.add_argument("--out-dir", type=Path,
                    default=Path("cluster_outputs/azure_admission_total_harm_sweep_plots"))
    ap.add_argument("--prefix", default="azure_admission_total_harm_sweep")
    args = ap.parse_args()
    ...
    plot_lines(df, "total_harm_ms",
               ...
               "Queue Pressure vs Max Total Harm",
               "admission_max_total_harm_ms",
               "Value",
    ...
    plot_lines(df, "total_harm_ms",
               ["slo_e2e_miss_rate"],
               ["E2E miss rate"],
               "E2E Miss Rate vs Max Total Harm",
               "admission_max_total_harm_ms",
               "Miss rate",
    ...
    plot_lines(df, "total_harm_ms",
               ...
               "Route Counts vs Max Total Harm",
               "admission_max_total_harm_ms",
               "Requests",
    ...
    plot_lines(df, "total_harm_ms",
               ...
               "Route Fractions vs Max Total Harm",
               "admission_max_total_harm_ms",
