...
OUT_DIR="${OUT_DIR:-cluster_outputs/bypass8_energy_guard_sweep}"
PLOTS_DIR="${PLOTS_DIR:-cluster_outputs/bypass8_energy_guard_sweep_plots}"
TRACE_PATH="${TRACE_PATH:-cluster_outputs/debug_trace_deterministic_bypass8.csv}"
...
PLOT_MAX_REQUEST_ID="${PLOT_MAX_REQUEST_ID:-7}"
...
if (($# > 0)); then
  ...
  GUARD_VALUES=(0 1 2 5 10 20 50 100)
fi
...
for guard_ms in "${GUARD_VALUES[@]}"; do
  ...
  echo "Running bypass8 energy-guard case: energy_latency_guard_ms=${guard_ms}"
...
"${PYTHON_BIN}" tools/plot_bypass8_energy_guard_sweep.py \
  --summary-dir "${OUT_DIR}" \
  --out-dir "${PLOTS_DIR}" \
  --prefix bypass8_energy_guard_sweep
